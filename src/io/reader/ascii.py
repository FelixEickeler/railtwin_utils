...
from src.io.formats import csf, CustomFrame
...
def read_ascii(src: Path, row_skip=0, delimiter="auto", point_format="auto", dimension_names="auto", offset="auto"):
    ...
    for dt in point_format.dtype().descr:
        if dt[0] in ["X", "Y", "Z", "x", "y", "z"]:
            input_dtypes.append(np.dtype("f8"))
            output_dtype.append(np.dtype(dt[1]))
            apply_offset_to.append(dt[0])
        else:
            input_dtypes.append(np.dtype(dt[1]))
    ...
    data.metadata["offset"] = offset[apply_offset_to]
    data = data.assign[apply_offset_to](data[apply_offset_to] - offset)
